...
while(cap.isOpened()): 
    ...
    results = model.predict(img, conf = 0.12 )
    new_frame_time = time.time() 
    fps = 1/(new_frame_time-prev_frame_time) 
    prev_frame_time = new_frame_time 
    cv2.putText(img,  '{0:.2f}'.format(fps), (7, 70), font, 1, (100, 255, 0), 2, cv2.LINE_AA)
    # coordinates
    for r in results:
        boxes = r.boxes
     ...
     #       print("Confidence --->",confidence)
      ...
      #      print("Class name -->", classNames[cls])
            ...
            cv2.putText(img, '{0:.1f}'.format(confidence*100), org1, font, fontScale, color, thickness)
